...
function attachPoint(r: Rect, side: Side, idx: number, count: number): { x: number; y: number } {
...
interface Route {
  path: string;
  /** tramo medio (para label de relación) */
  mid: { x1: number; y1: number; x2: number; y2: number };
  /** primer tramo (para cardinalidad origen) */
  first: { x1: number; y1: number; x2: number; y2: number };
  /** último tramo (para cardinalidad destino) */
  last: { x1: number; y1: number; x2: number; y2: number };
}

function route(a: { x: number; y: number }, b: { x: number; y: number }, exit: Side, lane: number, laneCount: number): Route {
...
/* ------------------------------------------------------------------ */
/* Labels con máscara (gap 6-10px sobre la línea)                      */
/* ------------------------------------------------------------------ */

function labelAbove(text: string, x: number, lineY: number, fill: string, bg: string, gap = 14): string {
...
function labelRight(text: string, lineX: number, y: number, fill: string, bg: string, gap = 12): string {
  const w = Math.max(12, text.length * 6.5 + 8);
  const lx = lineX + gap;
  return [
...
function entitySvg(e: Entity, r: Rect, C: ThemeColors): string {
  ...
  // máscara opaca + caja
  out.push(`<rect x="${r.x}" y="${r.y}" width="${r.w}" height="${r.h}" rx="6" fill="${C.paper}"/>`);
  out.push(`<rect x="${r.x}" y="${r.y}" width="${r.w}" height="${r.h}" rx="6" fill="${fill}" stroke="${stroke}" stroke-width="1"/>`);
  ...
  // nombre + tabla
  out.push(`<text x="${cx}" y="${r.y + 13}" fill="${C.ink}" font-size="12" font-weight="600" font-family="${SANS}" text-anchor="middle">${esc(e.name)}</text>`);
  out.push(`<text x="${cx}" y="${r.y + 31}" fill="${C.muted}" font-size="9" font-family="${MONO}" text-anchor="middle">${esc(e.table)}</text>`);

  // divisor
  out.push(`<line x1="${r.x}" y1="${r.y + 40}" x2="${r.x + r.w}" y2="${r.y + 40}" stroke="${C.hairline}" stroke-width="0.8"/>`);
  ...
  e.fields.forEach((f, i) => {
    ...
    out.push(`<text x="${r.x + 12}" y="${fy}" fill="${color}" font-size="9" font-family="${MONO}">${esc(prefix + f.name)}</text>`);
  });
...
export function exportHtml(diagram: Diagram, colors?: ThemeColors): string {
  ...
  // bounds + normalización a grilla de 4px
  const pad = 40;
  const legendH = 60;
  let minX = Infinity;
  let minY = Infinity;
  ...
  const tr = (r: Rect): Rect => ({ x: r.x - offX, y: r.y - offY, w: r.w, h: r.h });
  const tP = (p: { x: number; y: number }): { x: number; y: number } => ({ x: p.x - offX, y: p.y - offY });

  // resolver lados por relación: campo/lado explícito, o auto según posición relativa
  interface RelRoute {
    exit: Side;
    enter: Side;
  ...
  for (const rel of diagram.relations) {
    ...
    const sFieldIdx =
      srcEntity && rel.sourceFieldId ? srcEntity.fields.findIndex((f) => f.id === rel.sourceFieldId) : -1;
    const dFieldIdx =
      dstEntity && rel.targetFieldId ? dstEntity.fields.findIndex((f) => f.id === rel.targetFieldId) : -1;
    const [autoExit, autoEnter] = sideBetween(sr, dr);
    const exit: Side =
      sFieldIdx !== -1 || rel.sourceSide ? (rel.sourceSide === 'L' ? 'left' : 'right') : autoExit;
    const enter: Side =
      dFieldIdx !== -1 || rel.targetSide ? (rel.targetSide === 'R' ? 'right' : 'left') : autoEnter;
    relRoute.set(rel.id, {
      exit,
      enter,
  ...
  // fan-out por lado de origen y destino (attach points >= 12px); los extremos anclados a un campo no consumen fan-out
  const srcFan = new Map<string, number>();
  const dstFan = new Map<string, number>();
  for (const rel of diagram.relations) {
    const rt = relRoute.get(rel.id);
    if (!rt) continue;
    if (rt.sFieldIdx === null) srcFan.set(`${rel.source}|${rt.exit}`, (srcFan.get(`${rel.source}|${rt.exit}`) ?? 0) + 1);
    if (rt.dFieldIdx === null) dstFan.set(`${rel.target}|${rt.enter}`, (dstFan.get(`${rel.target}|${rt.enter}`) ?? 0) + 1);
  }

  // ---- aristas (antes que cajas) ----
  const edgeParts: string[] = [];
  const labelParts: string[] = [];

  // lanes por par (origen, lado, destino, lado) para rutas paralelas
  ...
  for (const rel of diagram.relations) {
    ...
    const a =
      sFieldIdx !== null
        ? { x: exit === 'left' ? sr.x : sr.x + sr.w, y: fieldRowY(sr, sFieldIdx) }
        : attachPoint(sr, exit, sIdx, srcFan.get(srcKey)!);
    const b =
      dFieldIdx !== null
        ? { x: enter === 'left' ? dr.x : dr.x + dr.w, y: fieldRowY(dr, dFieldIdx) }
    ...
    edgeParts.push(`<path d="${path}" fill="none" stroke="${C.muted}" stroke-width="1" marker-end="url(#arrow)"/>`);

    // label de relación en el tramo medio
    const midHoriz = midT.y1 === midT.y2;
    if (rel.label) {
      if (midHoriz) {
        labelParts.push(labelAbove(rel.label.toUpperCase(), (midT.x1 + midT.x2) / 2, midT.y1, C.soft, C.paper));
      } else {
        labelParts.push(labelRight(rel.label.toUpperCase(), midT.x1, (midT.y1 + midT.y2) / 2, C.soft, C.paper));
      }
    }

    // cardinalidad origen en el primer tramo
    const fHoriz = firstT.y1 === firstT.y2;
    if (fHoriz) {
      labelParts.push(labelAbove(rel.sourceCard, (firstT.x1 + firstT.x2) / 2, firstT.y1, C.muted, C.paper));
    } else {
      const downward = firstT.y2 > firstT.y1;
      labelParts.push(labelRight(rel.sourceCard, firstT.x1, (firstT.y1 + firstT.y2) / 2 + (downward ? 14 : 0), C.muted, C.paper));
    }

    // cardinalidad destino en el último tramo
    const lHoriz = lastT.y1 === lastT.y2;
    if (lHoriz) {
      labelParts.push(labelAbove(rel.targetCard, (lastT.x1 + lastT.x2) / 2, lastT.y1, C.muted, C.paper));
    } else {
      const downward = lastT.y2 > lastT.y1;
      labelParts.push(labelRight(rel.targetCard, lastT.x1, (lastT.y1 + lastT.y2) / 2 + (downward ? 14 : 0), C.muted, C.paper));
    }
  }

  // ---- nodos ----
  const nodeParts = diagram.entities.map((e) => entitySvg(e, tr(rects.get(e.id)!), C));
  ...
  if (diagram.relations.length > 0) {
    legend.push(`<text x="400" y="${legendY - 28}" fill="${C.soft}" font-size="8" font-family="${MONO}">1:1 uno a uno</text>`);
    legend.push(`<text x="540" y="${legendY - 28}" fill="${C.soft}" font-size="8" font-family="${MONO}">1:N uno a muchos</text>`);
  }
  if (diagram.entities.some((e) => e.focal)) {
    legend.push(`<rect x="680" y="${legendY - 36}" width="8" height="8" rx="2" fill="${C.accentTint}" stroke="${C.accent}" stroke-width="1"/>`);
    legend.push(`<text x="700" y="${legendY - 28}" fill="${C.soft}" font-size="8" font-family="${MONO}">raíz del dominio</text>`);
  }
...
<body>
  <div class="frame">
    ...
    <svg viewBox="0 0 ${W} ${H}" xmlns="http:      <title id="diagram-title">${esc(title)}</title>
