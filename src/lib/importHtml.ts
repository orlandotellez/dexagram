import type { Diagram, Entity, Field, Relation, Side } from './model.ts';
...
function sideOfPoint(b: Box, p: XY): Side | 'T' | 'B' | undefined {
  ...
  if (Math.abs(p.y - b.y) <= eps && p.x >= b.x - eps && p.x <= b.x + b.w + eps) return 'T';
  if (Math.abs(p.y - (b.y + b.h)) <= eps && p.x >= b.x - eps && p.x <= b.x + b.w + eps) return 'B';
  return undefined;
}

function pathBBox(d: string, start: XY, end: XY): { x1: number; y1: number; x2: number; y2: number } {
...
export function parseErHtml(html: string): Diagram {
  ...
  for (const r of rects) {
    seen.set(`${Math.round(r.x)},${Math.round(r.y)},${Math.round(r.w)},${Math.round(r.h)}`, r);
  }
  ...
  for (const b of seen.values()) {
    const id = uid();
    const focal = /#cb5bce/i.test(b.stroke) || /cb5bce|203,\s*91,\s*206/i.test(b.fill);
    ...
    const name =
      inner.find((t) => t.getAttribute('font-size') === '12')?.textContent?.trim() ??
      `Entidad ${entities.length + 1}`;
    const table =
      inner.find((t) => t.getAttribute('font-size') === '9' && t.getAttribute('text-anchor') === 'middle')
        ?.textContent?.trim() ?? '';
    const fieldEls = inner
      .filter((t) => t.getAttribute('font-size') === '9' && t.getAttribute('text-anchor') !== 'middle')
      .sort((a, z) => num(a.getAttribute('y')) - num(z.getAttribute('y')));
    const fields: Field[] = fieldEls.map((t) => {
      const raw = t.textContent ?? '';
      const isPk = raw.startsWith('# ');
      const isFk = raw.startsWith('→ ') || raw.startsWith('\u2192 ');
      return {
        id: uid(),
        name: raw.replace(/^#\s*/, '').replace(/^→\s*/, '').replace(/^\u2192\s*/, '').trim() || 'campo',
        isPk,
        isFk,
      };
    });
  ...
  if (entities.length === 0) throw new Error('No se encontraron entidades en el archivo');
  ...
  const snapField = (entity: Entity, b: Box, p: XY): string | undefined => {
    if (sideOfPoint(b, p) !== 'L' && sideOfPoint(b, p) !== 'R') return undefined;
    for (let i = 0; i < entity.fields.length; i++) {
      if (Math.abs(p.y - (b.y + 55 + i * 16)) <= 4) return entity.fields[i].id;
    }
    return undefined;
  };
  ...
  for (const p of paths) {
    ...
    const near = labels.filter((l) => l.x >= bb.x1 - 40 && l.x <= bb.x2 + 40 && l.y >= bb.y1 - 40 && l.y <= bb.y2 + 40);
    const relLabel = near.find((l) => l.isRel)?.text ?? '';
    const cards = near.filter((l) => !l.isRel && l.text !== relLabel);
    const srcCard = cards.length > 0 ? [...cards].sort((a, z) => dist(a, start) - dist(z, start))[0] : null;
    const dstCard =
      cards.filter((c) => c !== srcCard).length > 0
        ? [...cards.filter((c) => c !== srcCard)].sort((a, z) => dist(a, end) - dist(z, end))[0]
        : null;
    ...
    relations.push({
      ...
      sourceSide: sSide === 'L' || sSide === 'R' ? sSide : undefined,
      targetSide: dSide === 'L' || dSide === 'R' ? dSide : undefined,
    });
  }

  return {
    title: doc.querySelector('h1')?.textContent?.trim() ?? '',
    subtitle: doc.querySelector('.subtitle')?.textContent?.trim() ?? '',
    entities,
    relations,
  };
}
