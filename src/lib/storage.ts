import type { Diagram, Group } from './model.ts';
import { seedSalud, uid } from './model.ts';

const LS_LIST = 'diagram-web:diagrams';
const LS_GROUPS = 'diagram-web:groups';
const LS_LEGACY = 'diagram-web:diagram';
...
export function listDiagrams(): Diagram[] {
  ...
  if (legacy && typeof legacy === 'object' && Array.isArray(legacy.entities)) {
    ...
    const migrated = [{ ...legacy, id: legacy.id ?? `d-${Date.now().toString(36)}`, savedAt: Date.now() }];
    writeList(migrated);
    return migrated;
  }
  return [];
...
function readGroups(): Group[] {
  ...
  if (!parsed || !Array.isArray(parsed)) return [];
  return parsed.filter((g) => g && typeof g === 'object' && typeof g.id === 'string' && typeof g.name === 'string');
}
...
export function findDiagramBySlugOrId(list: Diagram[], slugOrId: string): Diagram | null {
  return (
...
export function downloadFile(name: string, content: string, mime = 'text/html'): void {
